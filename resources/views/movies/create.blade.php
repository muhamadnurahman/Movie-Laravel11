    ...
    <h2 class="text-2xl font-bold mb-6">Add Movie</h2>
        ...
        <form class="space-y-6" action="{{ route('movie.store') }}" method="POST">
            ...
            <div>
                ...
                <textarea name="description" id="description" class="w-full p-2 bg-gray-800 border border-gray-700 rounded focus:outline-none focus:ring-2 focus:ring-blue-600 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
